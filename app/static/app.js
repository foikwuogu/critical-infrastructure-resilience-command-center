...
function formatScenario(result){return `Scenario impact\nBefore: ${formatScore(result.before)}\nAfter: ${formatScore(result.after)}\nChange: ${formatScore(result.change)} points\n\nDecision rule\n${result.explanation.decision_rule}\n\nHuman review required: ${result.explanation.human_review_required?"Yes":"No"}`}
...
function renderAssets(){
 ...
 tb.innerHTML=rows.map(a=>{const rc=riskClass(a);return `<tr><td><b>${a.name}</b><br><span>${a.asset_type}</span></td><td>${a.zone}</td><td>${a.criticality}</td><td>${formatScore(a.health)}</td><td><span class="risk-badge ${rc} ${rc}-bg">${(a.failure_probability*100).toFixed(1)}%</span></td><td>${formatScore(a.rul_hours)}h</td><td>${a.crypto_algorithm}</td><td>${a.pqc_status} (${formatScore(a.pqc_readiness)})</td><td>${formatScore(a.identity_trust)}</td></tr>`}).join("")||`<tr><td colspan="9">No assets match the current filter.</td></tr>`;
...
async function refresh(){
 try{
  const r=await api("/api/resilience");
    $("overall").textContent=formatScore(r.overall);$("cyber").textContent=formatScore(r.cyber);$("physical").textContent=formatScore(r.physical);$("pqc").textContent=formatScore(r.pqc);$("availability").textContent=formatScore(r.availability)+"%";
  ...
  setbar("barCyber",r.cyber);setbar("barPqc",r.pqc);setbar("barPhysical",r.physical);setbar("barAvailability",r.availability);
    $("txtCyber").textContent=formatScore(r.cyber);$("txtPqc").textContent=formatScore(r.pqc);$("txtPhysical").textContent=formatScore(r.physical);$("txtAvailability").textContent=formatScore(r.availability);
  currentAssets=await api("/api/assets");renderAssets();
    const m=await api("/api/migration"),mq=$("migrationContent");mq.innerHTML=`<p>Legacy: <b>${m.legacy}</b> &nbsp; Planned: <b>${m.planned}</b> &nbsp; Migrated: <b>${m.migrated}</b></p>`+m.priority.map(x=>`<div class="queue"><div class="queue-head"><b>${x.asset}</b><span>${formatScore(x.readiness)}</span></div><small>${x.algorithm} | criticality ${x.criticality}</small></div>`).join("");
